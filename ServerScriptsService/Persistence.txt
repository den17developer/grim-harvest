-- Persistence.lua
-- Безопасные загрузка/сохранение, батчевый Flush, совместимость с Save(plr, save)
...
function M.Load(plr: Player): table?
	...
	if not ok then
		warn("[Persistence] Load failed for", plr, err)
		return nil
	end
	if data == nil then
		return nil
...
function M.FlushNow(plr: Player, buildSaveFn: (() -> table)?)
	...
	-- (опционально) подождать бюджет:
	-- while DataStoreService:GetRequestBudgetForRequestType(Enum.DataStoreRequestType.SetAsync) <= 0 do
	-- 	task.wait(0.25)
	-- end
	...
	if not ok then
		warn("[Persistence] FlushNow failed for", plr, err)
